...
const API_KEY = process.env.OPENROUTER_API_KEY || process.env.VITE_OPENROUTER_API_KEY || process.env.OPENAI_API_KEY;
...
const getMockQuestions = (position, difficulty, count) => {
  const normalizedPosition = position.toLowerCase().includes('frontend') ? 'frontend developer' : 'backend developer';
  const normalizedDifficulty = difficulty.toLowerCase() || 'easy';
  const questions = mockInterviewQuestions[normalizedPosition]?.[normalizedDifficulty] || mockInterviewQuestions['frontend developer']['easy'];
  const shuffled = questions.sort(() => 0.5 - Math.random());
  return shuffled.slice(0, count);
};
...
export const generateInterviewQuestions = async (role = 'software developer', difficulty = '', count = 5) => {
  ...
  return getMockQuestions(role, difficulty, count);
};
...
export const evaluateAnswer = async (question, answer) => {
  ...
  if (API_KEY) {
    try {
      ...
      console.warn('AI evaluation failed, using mock evaluation:', error.message);
    }
  }
